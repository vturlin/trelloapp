...
class New_card():
    ...
    def __init__(self,title,desc,idList,idMembers,idlabels,dueDate):
        self.title = title
        self.desc = desc
...
class Trello():
    ...
    # other constants 
    LABELS_TO_CHECK = ['GTM', 'GA4', 'PREZ', 'SUPPORT', 'LOOKER STUDIO', 'PRESTA', 'INTERNE', 'PDM', 'PRIVACY']
    ...
    def _card_description_df(self):
        ...
        description_elements.columns = ['client', 'estimated_spent_time', 'final_time_spent', 'proposal_price', 'sale_price','rest']
    ...
    def _cards_df(self) -> pd.DataFrame:
        ...
        final_columns = ['card_name', 'due_date', 'start_date','id_members', 'id_list', 
           'items_completed', 'items_amount', 'GTM', 'GA4','PREZ', 'SUPPORT', 
           'LOOKER STUDIO', 'PRESTA', 'INTERNE', 'PDM','PRIVACY', 'client', 
        ...
        df.drop(columns=['labels','desc'], inplace=True)
